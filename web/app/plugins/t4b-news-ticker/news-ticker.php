...
function t4bnt_enqueue_scripts() {
	...
	if($t4bnt_enable == 'on') {
		if($ticker_effect == 'scroll') {
			wp_register_script('liscroll', WP_PLUGIN_URL.'/t4b-news-ticker/assets/js/jquery.liscroll.js', array('jquery'), '1.2.3');
			wp_enqueue_script('liscroll');
		} else {
			wp_register_script('ticker', WP_PLUGIN_URL.'/t4b-news-ticker/assets/js/jquery.ticker.js', array('jquery'), '1.2.3');
			wp_enqueue_script('ticker');
		}
		if($ticker_effect == 'scroll') {
			wp_enqueue_style('t4bnewsticker', WP_PLUGIN_URL.'/t4b-news-ticker/assets/css/t4bnewsticker.css?v=1.2.3');
		} else {
			wp_enqueue_style('tickerstyle', WP_PLUGIN_URL.'/t4b-news-ticker/assets/css/ticker-style.css?v=1.2.3');
		}
	}
}
